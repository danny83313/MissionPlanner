...
namespace PathProgram
{
    public class PathProgramming
    {
        ...
        static int intabu_noch = 50;//單路徑_持續未改善終結代數
        static int crstabu_noch = 100;//多路徑_持續未改善代數
        static double exnofly_gain = 1.2;//禁航區擴大倍率(轉折點)
                         ...
                         ref List<PointLatLngAlt> Cpointlist,ref List<PointLatLngAlt> Dpointlist, ref List<PointLatLngAlt> Epointlist)
        {
            ...
            List<PointLatLngAlt> turn_list = new List<PointLatLngAlt>();
            Apointlist.Clear();
            Bpointlist.Clear();
            Cpointlist.Clear();
            Dpointlist.Clear();
            ...
            Array.Copy(improve(), opt_multi_path, opt_multi_path.Length);//tabu後解
            ...
            if (dll_noflylist.Count != 0)
            {
                ...
                {//轉折加在目標點list後
                    dll_goallist.Add(new PointLatLngAlt(dll_exnoflylist[i].Lat, dll_exnoflylist[i].Lng, dll_exnoflylist[i].Alt));
                }
            }

            for (int i = 0; i < m_num; i++)
            {//輸出
                for (int j = 0; j < dll_goallist.Count + 3 - m_num; j++)
                {//演算後 點轉經緯
                ...
                for (int j = 0; j < turn_list.Count; j++)
                {
                    ...
                    if ((turn_list[j].Lat == turn_list[j + 1].Lat) && (turn_list[j].Lng == turn_list[j + 1].Lng)) break;
                }
        ...
        static private int[,] improve()//改良後路徑(產生最終解)/i:/o:最終路徑 ******
        {
        ...
        static private int[,] initail()//分割後 初始路徑/i:/o:分割後初始群路徑
        {
        ...
        static private int[,] cut(int[] single_path)//路線分割//i:全部目標點/o:各群分割後路徑
        {
        ...
        static private int insert()//tabu後路徑插入禁航區/i:/o:最終路徑插入轉折
        {
            for (int i = 0; i < m_num; i++)
            {
        ...
        static private int[,] Tabu_cross_route(int[,] multi_path)//多路徑TABU/i:單路徑TABU後，分割之初始路徑/o:三群路徑二維
        {
            ...
            for (int i = 0; i < m_num; i++)
            {
                ...
                double dist_f = path_distance(path_global);//計算交換過程的單群距離
            ...
            double global_sub_max = double.MaxValue;//全域次佳
            ...
            for (int i = 0; i < m_num; i++)
            {
                ...
                double dist_initail = path_distance(path_initail);//計算交換過程的單群距離
                initail_total += dist_initail;//持續累加總距離
                if (dist_initail > initail_max) initail_max = dist_initail;   //新找到>最大距離 新取代舊
            }
            ...
            do
            {
                ...
                if (compare_max_total(near_opt_max, near_opt_total, global_opt_max, global_opt_total))//鄰近解較全域解優->取代
                {
        ...
        static private int[] Tabu_in_route(int[] ndm) //單路徑tabu//i:/o:
        {
            ...
            } while (!(no_change == intabu_noch));//終止條件 X代未改善

            return sole_opt_path;
        }

        static private double path_distance(int[] path) //初始解距離/i:/o:
        {
            double value;
        ...
        static private int[] NDM() //最近距離法//回傳路徑/i:/o:
        {//把row選完=選完全部目標點 即路徑
            ...
            for (int i = 0; i < latLength - 1; i++)
            {
                ...
                for (int j = 0; j < latLength; j++)
                {
                    ...
                    {
                        if (Cij_copy[j, k] < min_value)//找最小值
                        {
        ...
        static private bool compare_max_total(double a_max, double a_total, double b_max, double b_total)//(鄰近最佳max,total,全域最佳max,total)/i:當前解 全域解/o:TF
        {
            bool ask_tf = false;
        ...
        static private double[,] distance() //距離陣列//astar/i:/o:A*後距離陣列
        {
            int count = 0;
            for (int i = 0; i < Cij.GetLength(0); i++)
            {
                for (int j = 0; j < Cij.GetLength(0); j++)
                {
                    ...
                    else
                    {
                        ...
                        save_turning_point[count, 0] = i;//起
                        save_turning_point[count, 1] = j;//終
                        //save_turning_point[,起點.終點.轉折]
                        int f = 1;
                        do
                        {
                            f++;
        ...
        static private bool crsnofly(double lat0, double lng0, double lat1, double lng1, double lat2, double lng2, double lat3, double lng3)//判斷路線是否交叉nofly/i:/o: ***
        {
            {
        ...
        static private double crsproduct(double lat0, double lng0, double lat1, double lng1, double noflylat0, double noflylng0) //叉積 正負號/i:/o:
        {
            return ((noflylat0 - lat0) * (lng1 - lng0)) - ((lat1 - lat0) * (noflylng0 - lng0));
        }

        static private bool sameline(double lat0, double lng0, double lat1, double lng1, double noflylat0, double noflylng0) //共線判斷/i:/o:
        {
        ...
        static private double dist(double lat0, double lng0, double lat1, double lng1)//距離function/i:任意兩點/o:兩點間距離
        {
        ...
        static private double exnofly()//擴大禁航區-轉折點/o:轉折點(經緯)
        {//dll_noflylist.Sum(sumlat => sumlat.Lat) 
            ...
            double pclng = ((dll_noflylist.Sum(sumlng => sumlng.Lng)) - (dll_noflylist[dll_noflylist.Count - 1].Lng)) / (dll_noflylist.Count - 1);
            for (int i = 0; i < dll_noflylist.Count - 1; i++)
            {
                dll_exnoflylist.Add(new PointLatLngAlt(pclat + exnofly_gain * (dll_noflylist[i].Lat - pclat), pclng + exnofly_gain * (dll_noflylist[i].Lng - pclng)));// pc+a*(pi-pc) 擴大X倍
            }
            return 0;
        }

        static private double astar(double lat0, double lng0, double lat1, double lng1) //A*/i:起點終點(經緯)/o:繞過nofly距離
        {
            //(起點,轉折1~4,目標點)
            
            List<PointLatLngAlt> dll_turnlist = new List<PointLatLngAlt>();

            dll_turnlist.Add(new PointLatLngAlt(lat0, lng0, 100));//起點
            ...
            dll_turnlist.Add(new PointLatLngAlt(lat1, lng1, 100));//目標
            ...
            //
            double[,] open = new double[nflatLength * nflatLength + latLength, nflatLength * nflatLength + latLength]; //{ { 0, 0, 0, 0 } };//arr.row0=0,0,0,0
        ...
        static private double[,] smalldistance(double[] slat, double[] slng)//原點.轉折點.目標點陣列/i:/o:二維距離矩陣
        {
            double[,] cij = new double[nflatLength + 2, nflatLength + 2];
        ...
        static private double[,] delete(double[,] array, int c) //取出後刪除row//i:/o:
        {
        ...
        static private double[,] into(double[,] array2d, double[] array1d) //A* add open close下方列//i:/o:
        {
